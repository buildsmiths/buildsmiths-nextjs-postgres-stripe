...
export default async function DevStatusChips() {
    ...
    const chip = (ok: boolean, text: string, warn?: boolean) => (
        <span className={`px-2 py-0.5 rounded ${ok ? 'bg-green-100 text-green-800' : warn ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>{text}</span>
    );

    return (
        <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className={`px-2 py-0.5 rounded ${env === 'production' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>{env}</span>
